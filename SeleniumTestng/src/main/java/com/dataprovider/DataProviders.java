...
import com.utilities.NewExcelLibrary;
	...
	public Object[][] getCredentials() {
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				//data[i][j]=obj.readExcel("Credentials", )
				data[i][j] = obj.getCellData("Credentials", j, i + 2);
			}
		}
		return data;
	}
	@DataProvider(name = "VerifyTable")
	public Object[][] verifyWebTableData() {
		// Totals rows count
		int rows = obj.getRowCount("VerifyTable");
		// Total Columns
		int column = obj.getColumnCount("VerifyTable");
		int actRows = rows - 1;
		...
		for (int i = 0; i < actRows; i++) {
			for (int j = 0; j < column; j++) {
				//data[i][j]=obj.readExcel("Credentials", )
				data[i][j] = obj.getCellData("VerifyTable", j, i+1);
			}
		}
		return data;
	}
